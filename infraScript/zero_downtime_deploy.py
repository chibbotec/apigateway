...
class ServiceManager:
    # 초기화 함수
    def __init__(self, socat_port: int = 9000, sleep_duration: int = 3) -> None:
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'apigateway_2', self.services['apigateway_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/chibbotec/apigateway' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/chibbotec/apigateway' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:9000 -e TZ=Asia/Seoul -v /dockerProjects/chibbotec/apigateway/volumes/gen:/gen --pull always ghcr.io/chibbotec/apigateway")
    ##
